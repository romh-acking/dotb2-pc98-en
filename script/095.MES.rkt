(mes
 ...
 (seg (? (= P 5) (= 199 #t) (= 201 #t) (= 204 #t))
   ...
   (wait)
   (text-reset 1)
   (text "Cole: That's right. Now that you mention it, that's the onlyother way to open the door..." 'br)
   (text "*Click*")
   (wait)
   (text-reset 1)
   (text "Cole: ?")
 ...
 (seg (? (= P 8) (= 204 #f))
   ...
   (wait)
   (text-reset 1)
   (text "Steve: The door's too sturdy, it's not like we can use a gunto shoot the lock.")
   (set-reg 204 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 8))
