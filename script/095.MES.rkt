...
(str "impossible.")
        ...
        (wait)
        (text-reset 1))
       (// (str "Cole: Damn, the lab's been overrun...") (wait) (text-reset 1))))
 ...
 (seg (? (= P 5) (= 199 #t) (= 201 #t) (= 204 #t))
   ...
   (wait)
   (text-reset 1)
   (str "Cole: That's right. Now that you mention it, that's the only" 'br)
(str "other way to open the door..." 'br)
   (str "*Click*")
   (wait)
   (text-reset 1)
   (str "Cole: ?")
...
(str "deactivating the door lock.")
   ...
   (wait)
   (text-reset 1)
   (str "Steve: The door's too sturdy, it's not like we can use a gun" 'br)
(str "to shoot the lock.")
   (set-reg 204 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 8))
