(mes
 ...
 (seg (? (= P 6))
   ...
   (text-reset 1))
 (seg (? (= P 7) (= 176 #f))
   (text "Cole: There's debris all over the floor. I'd better watch mystep, or I'd slip and fall, and it's tetanus time.")
   (set-reg 176 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 7) (= 177 #f))
   ...
   (text-reset 1))
 (seg (? (= P 7) (= 178 #f))
   (text "Cole: I'm sure the police cleanup crew took care of the     wreckage, but I bet they never figured an android was behindall this mayhem.")
   (set-reg 178 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 7) (= 179 #f))
