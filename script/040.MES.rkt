(mes
 ...
 (seg (? (= P 5) (= 118 #f))
   (text "Cole: Judging by the lockers, this must be the lab's waitingroom.")
   (set-reg 118 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 5) (= 119 #f))
   (text "Cole: All these lockers are locked. I'm not getting into anyof them. That's how lockers work.")
   (set-reg 119 #t)
   (wait)
   (text-reset 1))
 ...
 (seg (? (= P 7) (= 121 #f))
   (text "Cole: There's a desk on the left, and the pipe-frame bed I'msitting on.")
   (set-reg 121 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 7) (= 122 #f))
