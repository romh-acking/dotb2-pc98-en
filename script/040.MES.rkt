(mes
 ...
 (seg*
  ...
  (if (</>
       (//
        ...
        (wait)
        (text-reset 1))
       (// (? (= 146 #t)) (str "Cole: This is my room at the Perain Institute.") (wait) (text-reset 1))
       (//
        (? (= 116 #f))
 ...
 (seg (? (= P 5) (= 118 #f))
   (str "Cole: Judging by the lockers, this must be the lab's waiting" 'br)
(str "room.")
   (set-reg 118 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 5) (= 119 #f))
   (str "Cole: All these lockers are locked. I'm not getting into any" 'br)
(str "of them. That's how lockers work.")
   (set-reg 119 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 5)) (str "Cole: It's a row of locked lockers.") (wait) (text-reset 1))
 (seg (? (= P 6) (= 120 #f))
   (str "Cole: I came in through this door. It leads to the corridor.")
   (set-reg 120 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 6)) (str "Cole: A plain door with no lock.") (wait) (text-reset 1))
 (seg (? (= P 7) (= 121 #f))
   (str "Cole: There's a desk on the left, and the pipe-frame bed I'm" 'br)
(str "sitting on.")
   (set-reg 121 #t)
   (wait)
   (text-reset 1))
