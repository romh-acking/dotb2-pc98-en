(mes
 ...
 (seg*
  ...
  (if (</>
       (//
        (? (= 100 #f))
        (text "Cole: Huh? Sheila isn't here... Maybe she got sick of" 'br)
(text "waiting for me and went home.")
        (set-reg 100 #t)
        (wait)
        (text-reset 1))
 ...
 (seg (? (= P 5) (= 104 #t) (= 106 #t) (= 107 #t))
   (text "Cole: Wha? I didn't see it before since it was behind the" 'br)
(text "chair, but there's a memo on the table.")
   (wait)
   (text-reset 1)
   ...
   (text-reset 1)
   (text "To Cole" 'br)
   (text "Since I'll be getting home late anyway, I'm going to come" 'br)
(text "and look for you. If you beat me back, just wait for me -- I" 'br)
(text "promise I'll be there. --Sheila")
   (wait)
   (text-reset 1)
   (text "Cole: What's all this? She's going to look for me...where?" 'br)
(text "Oh man, did she go to Bar Rease all by herself...?")
   (wait)
   (text-reset 1)
   (text "Cole: Steve isn't going to be at Rease today, and I have no" 'br)
(text "idea what Bloody Fox might do to Sheila!")
   (wait)
   (text-reset 1)
 ...
 (seg (? (= P 6))
   (branch-random
    (</>
     (/ (text "Cole: The wall's plastered with the photos and clippings" 'br)
(text "I've pasted up."))
 ...
 (seg (? (= P 7) (= 102 #f))
   (text
    "Cole: The AC unit is off. The weather's been nice lately, so" 'br)
"it's been off for months.")
   (set-reg 102 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 7))
   (text "Cole: It'll probably get cold next month, so I'll probably" 'br)
(text "need to turn it on again.")
   (wait)
   (text-reset 1))
 (seg (? (= P 8) (= 103 #f))
   ...
   (text-reset 1))
 (seg (? (= P 8) (= 104 #f))
   (text "Cole: Sheila will get in touch with me when she gets home." 'br)
(text "She worries a lot...")
   (set-reg 104 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 8))
   (text "Cole: No need to call anyone. Sheila will get in touch" 'br)
(text "shortly.")
   (wait)
   (text-reset 1))
 (seg (? (= P 9) (= 105 #f))
   (text "Cole: The door was locked when I came back. Since Sheila has" 'br)
(text "my spare key, she probably locked it for safety's sake.")
   (set-reg 105 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 9) (= 106 #f))
   (text
    "Cole: It's too late to go out again. When the sun comes up" 'br)
"tomorrow I'll head back into the city to look for Purple Sky" 'br)
"again.")
   (set-reg 106 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 9))
   (text
    "Cole: I should just wait for Sheila to call me. Tomorrow" 'br)
"we'll both go to Steve's place to talk with him more.")
   (wait)
   (text-reset 1))
 (seg (? (= P 10) (= 107 #f))
   ...
   (text-reset 1))
 (seg (? (= P 10))
   (text "Cole: I want to think about the incident some more, so best" 'br)
(text "not to get distracted by the radio blaring whatever.")
   (wait)
   (text-reset 1))
 (seg (? (= P 2))
   (exec-mem 6064 2 2)
