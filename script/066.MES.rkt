; Dead of the Brain 2 (PC-98) - 066.MES
; Translated by Geometrizer
; Edited by JackDBS, commented by trentsignia
; --Description:--
; In the lab with Lisle.
(mes
 ...
 (seg*
  ...
  (if (</>
       (//
        (? (= 108 #t) (= 121 #f))						; Lead-in when getting Sheila water
        (str "Cole: Doctor Lisle, could I have a glass of water? Sheila" 'br)
        (str "is real thirsty...")
        (wait)
        (text-reset 1)
 ...
 (seg (? (= P 7))								; Lab #3 (repeat line)
   (str "Lisle: This room is my raison d'etre.")
   (wait)
   (text-reset 1))
 (seg (? (= P 1))								; MOVE
   (exec-mem 6064 2 1)
