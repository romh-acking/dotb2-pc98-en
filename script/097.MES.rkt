...
(str "again.")
        ...
        (text-reset 1))
       (//
        (str "Steve: Cole... Don't let this bring you down.")
        (wait)
        (text-reset 1)
 ...
 (seg (? (= P 5) (= 101 #f))
   (str "Cole: When I was talking with Doc, as odd as it might sound," 'br)
(str "I felt a real warmth in those words.")
   (set-reg 101 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 5))
...
(str "on the right ourselves.")
   ...
   (text-reset 1))
 (seg (? (= P 6))
   (str "Cole: Doctor Lisle once used this computer for his research." 'br)
(str "It's been battered so badly I doubt any data remains.")
   (wait)
   (text-reset 1))
 (seg (? (= P 7) (= 104 #f))
   (text
   ...
   (wait)
   (text-reset 1))
 (seg (? (= P 7)) (str "Cole: Is this the end of Dr. Lisle's research, I wonder...") (wait) (text-reset 1))
 (seg (? (= P 8) (= 101 #t) (= 107 #t))
   (str "Steve: Where exactly did Dr. Cooger drop the bag with the" 'br)
(str "DNV cure?")
   (wait)
   (text-reset 1)
   (str "Cole: At the cemetery in the neighboring town, while we were" 'br)
(str "getting chased by a zombie cop named Jack. ...I guess you" 'br)
(str "had to be there.")
   (wait)
   (text-reset 1)
...
(str "possible.")
   (wait)
   (text-reset 1)
   (str "Steve: But just going on the name 'Jill' will be tricky. How" 'br)
(str "to find him...")
   (set-reg 107 #t)
   (wait)
   (text-reset 1))
