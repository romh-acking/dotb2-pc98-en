...
(str "Pharmaceuticals used the reanimation serum to raise the dead" 'br)
(str "and level the city.")
   (set-reg 153 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 5) (= 154 #f))
...
"would leave the keys in the ignition.")
   ...
   (text-reset 1))
 (seg (? (= P 6))
   (str "Cole: If the car's still here, Steve's probably still inside" 'br)
(str "the building.")
   (wait)
   (text-reset 1))
 (seg (? (= P 7) (= 157 #f))
