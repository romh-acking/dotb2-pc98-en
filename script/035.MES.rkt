(mes
 ...
 (seg*
  ...
  (if (</>
       (//
        ...
        (text-reset 1))
       (//
        (str "Cole: This was the city I lived in... But look at it now!")
        (wait)
        (text-reset 1))))
  (loop
         ...
         (loop
          ...
          (if (</>
               (//
                ...
                (wait)
                (text-reset 1)
                (str "Cole: But this town's been so messed up by the zombie attack" 'br)
                (str "that he wouldn't have stuck around. If it were me, I'd take" 'br)
                (str "shelter somewhere far away...like the Perain Institute.")
                (wait)
                (text-reset 1)
 ...
 (seg (? (= P 8) (= 125 #f))
   ...
   (wait)
   (text-reset 1))
 (seg (? (= P 8)) (str "Cole: I can see tons of corpses inside the store.") (wait) (text-reset 1))
 (seg (? (= P 2))
   (exec-mem 6064 2 2)
   (menu1
    25
