(mes
        ...
        (//
         ...
         (loop
          ...
          (branch-var
           S
           (</>
            (/ (text "Cole: Right, so to find Sheila, I should try checking places where Steve might go."))
            (/ (text "Cole: Right, so to find Sheila, I should try checking places where Catherine might go."))
            (/ (text "Cole: Right, so to find Sheila, I should try checking places where Briar might go."))
            (/ (text "Cole: Right, so to find Sheila, I should try checking places where I...would go...? Wait...what?"))
          ...
          (if (</>
               (//
                ...
                (text-reset 1)
                (text
                 "Cole: But this town's been so messed up by the zombie attack that he wouldn't have stuck around. If it were me, I'd take shelter somewhere far away...like the Perain Institute.")
                (wait)
                (text-reset 1)
                (text
 ...
 (seg (? (= P 6) (= 114 #f))
   ...
   (text-reset 1))
 (seg (? (= P 6) (= 115 #f))
   (text "Cole: Where did all these zombies come from? There's so many of them, they just rolled over the town like a tidal wave! Made of blood!")
   (wait)
   (text-reset 1)
   ...
   (text-reset 1))
 (seg (? (= P 6) (= 116 #f))
   (text "Cole: Now that the undead are back, it's obvious someone's been messing with the reanimation serum. Who would do such a thing...")
   (set-reg 116 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 6) (= 117 #f))
   (text "Cole: I bet you cash money that the people using Doc's serum to make Purple Sky are the same people who made these zombies.")
   (set-reg 117 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 6))
 ...
 (seg (? (= P 7))
   ...
   (text-reset 1))
 (seg (? (= P 8) (= 123 #f))
   (text "Cole: Hey, that used to be a good store! And all it took was a few zombie assholes to ruin it for everyone.")
   (set-reg 123 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 8) (= 124 #f))
   (text "Cole: This store's profit margins are definitely gonna be in the red.")
   (set-reg 124 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 8) (= 125 #f))
