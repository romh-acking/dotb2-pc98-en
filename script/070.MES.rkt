 ...
 (seg*
  ...
  (if (</>
       (//
        ...
        (wait)
        (text-reset 1))
       (// (str "Cole: I really don't want to be here...") (wait) (text-reset 1))))
...
(str "distinctive smell of gray matter has been assaulting my nose" 'br)
(str "for a while now.")
   (wait)
   (text-reset 1))
 (seg (? (= P 6) (= 142 #f))
   (str "Cole: A female corpse is lying prone. I guess she was also a" 'br)
(str "zombie.")
   (set-reg 142 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 6) (= 143 #f))
   (str "Cole: I can't show any mercy on lady zombies. They'll attack" 'br)
(str "anyone indiscriminately, just like male zombies.")
   (set-reg 143 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 6) (= 144 #f))
...
(str "expected them to make it all the way from the city.")
   ...
   (text-reset 1))
 (seg (? (= P 6))
   (str "Cole: His brain's been destroyed. He ain't getting up again.")
   (wait)
   (text-reset 1))
 (seg (? (= P 7) (= 145 #f))
...
(str "wonder if any are left.")
 ...
 (seg (? (= P 8) (= 147 #f))
   ...
   (wait)
   (text-reset 1))
 (seg (? (= P 8)) (str "Cole: It looks like there aren't any zombies near the desk.") (wait) (text-reset 1))
 (seg (? (= P 9) (= 148 #f))
...
(str "locker...?")
 ...
 (seg (? (= P 9) (= 149 #f))
   ...
   (wait)
   (text-reset 1))
 (seg (? (= P 9)) (str "Cole: I really don't need to poke around the locker anymore." 'br)
(str "We've established the no-zombie thing.") (wait) (text-reset 1))
 (seg (? (= P 1))
   (exec-mem 6064 2 1)
