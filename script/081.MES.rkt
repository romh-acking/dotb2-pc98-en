...
"night, Steve and Lisle came here to exhume Doc's body...")
   ...
   (text-reset 1))
 (seg (? (= P 5) (= 102 #f))
   (str "Steve: Don't worry, there weren't any zombies lurking around" 'br)
(str "here last night.")
   (wait)
   (text-reset 1)
   (str "Cole: It looks like Daniel's meddling hasn't extended to the" 'br)
(str "cemetery...")
   (wait)
   (text-reset 1)
   (text
...
"cemetery.")
   ...
   (wait)
   (text-reset 1))
 (seg (? (= P 5)) (str "Cole: Is Doc really resting here in Disbell?") (wait) (text-reset 1))
 (seg (? (= P 6) (= 103 #f))
...
(str "classified if you steal from a zombie, I wonder...?")
   ...
   (wait)
   (text-reset 1))
 (seg (? (= P 6)) (str "Cole: The gate isn't locked.") (wait) (text-reset 1))
 (seg (? (= P 7) (= 104 #f))
...
(str "flashlight.")
 ...
 (seg (? (= P 8) (= 106 #f))
   ...
   (wait)
   (text-reset 1)
   (str "Steve: There's one flashlight.")
   (set-reg 106 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 8) (= 107 #f))
