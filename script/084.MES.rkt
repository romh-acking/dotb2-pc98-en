...
(str "cemetery, Cole helped me out.")
   ...
   (wait)
   (text-reset 1)
   (str "Steve: Cole's saved my bacon twice now. I thank you again.")
   (wait)
   (text-reset 1)
   ...
   (text-reset 1))
 (seg (? (= P 6) (= 123 #f))
   (str "Lisle: How did you kill the zombies with such efficiency?")
   (wait)
   (text-reset 1)
...
(str "they dropped immediately.")
   (wait)
   (text-reset 1)
   (str "Lisle: Come to think of it, the woman's corpse on 1F did" 'br)
(str "have an entry wound, dead center in the temple.")
   (wait)
   (text-reset 1)
...
"care of Daniel.")
   ...
   (wait)
   (text-reset 1)
   (str "Steve: No...there's no need to rush. We'll take care of" 'br)
...
(str "fro?")) (/ (str "Lisle: What's the matter? Why are your eyes darting to and" 'br)
(str "fro?"))))
   (wait)
   (text-reset 1))
 (seg (? (= P 1) (= 901 #t) (= 120 #f))
