(mes
 ...
 (seg*
  ...
  (if (</>
       (//
        (? (= 180 #f))
        (text "B.F.: Hold still, bitch!")
        (wait)
        (text-reset 1)
        (text "Woman: No, stop!")
        (wait)
        (text-reset 1)
        (text "Cole: It's Bloody Fox!")
        (set-reg 180 #t)
        (wait)
        (text-reset 1))
       (//
        (text "B.F.: Shut the fuck up!")
        (wait)
        (text-reset 1)
        (text "Woman: No!!!")
        (wait)
 ...
 (seg (? (= P 5) (= 181 #f))
   (text "Cole: The Bloody Fox members are assaulting that woman..." 'br)
(text "And I've gotta get answers out of them.")
   (set-reg 181 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 5) (= 182 #f))
   (text "Cole: Wait, is that guy holding a syringe? What's going on" 'br)
(text "here?")
   (wait)
   (text-reset 1)
   (text "Cole: The Bloody Fox members are assaulting that woman..." 'br)
(text "And I've gotta get answers out of them.")
   ...
   (text-reset 1))
 (seg (? (= P 5))
   (text "Cole: Hey! What's in that syringe!?")
   (wait)
   (text-reset 1)
   (text "B.F.: Huh? Who the fuck are you?")
   (wait)
   (text-reset 1)
   (text "Cole: Who I am doesn't matter! Tell me what's in there!")
   (wait)
   (text-reset 1)
   (text "Woman: Help me! Please!!!")
   (wait)
   (text-reset 1)
   (text "B.F.: Heh heh heh... You wanna know, you come and get it!")
   (wait)
   (text-reset 1)
   (text "Cole: I guess it's come to this, then...freeze, punks!")
   (wait)
   (text-reset 1)
   (text "B.F.: Whu? You think a little pea-shooter like that's gonna" 'br)
(text "spook us?")
   (wait)
   (text-reset 1)
   (text "Woman: Help me!!!")
   ...
   (sound '|| 1)
   (text "B.F.: Forget about the girl for now, let's show this" 'br)
(text "shitfucker how the Bloody Wolves deal with uninvited guests!")
   (wait)
   (text-reset 1)
   ...
   (if (</>
        ...
        (//
         (? (= P 7))
         (text "B.F.: Accept your ass-kickin'!")
         (wait)
         (text-reset 1)
         (exec-mem 14544 2 2))
        (//
         (? (= P 8))
         (text "B.F.: You're fucked now!")
         (wait)
         (text-reset 1)
         (exec-mem 14544 2 2))
        (//
         (? (= P 9))
         (text "B.F.: Look at me when I'm kicking your ass!")
         (wait)
         (text-reset 1)
         (exec-mem 14544 2 2))
        (//
         (? (= P 10))
         (text "B.F.: Who taught you how to fight?")
         (wait)
         (text-reset 1)
         (exec-mem 14544 2 2))
        (//
   ...
   (if (</>
        (//
         ...
         (branch-random
          (</>
           (/ (text "You're a hundred years too early to be pickin' a fight with" 'br)
(text "Bloody Fox!"))
           (/ (text "Tell your friends about how tough Bloody Fox is... Tell" 'br)
(text "them...IN HELL!"))))
         (wait)
         (text-reset 1)
         (exec-mem 17408 3)
        ...
        (//
         ...
         (if (</>
              ...
              (//
               ...
               (image-mem 1 3)
               (exec-mem 9920 1 6 32 113 32 6 6 113 64 8 50)
               (text "B.F.: You're quick, I'll give you that... But now it's time" 'br)
(text "for you to join the CLUB!")
               (wait)
               (text-reset 1)
               (text "B.F.: Die, fuckwit!")
               (wait)
               (text-reset 1)
               (exec-mem 14544 2 2)
               (text "Cole: Hyaaaaaaa!")
               ...
               (text-reset 1)
               (exec-mem 9920 2 6 6 113 64 6 32 113 32 0 50)
               (text "B.F.: Eat it, you bastard!")
               (wait)
               (text-reset 1)
               (text
                "B.F.: So who was that dude packing heat? He was asking about" 'br)
"Purple Sky...you figure he's a pig?")
               (wait)
               (text-reset 1)
               (text
                "B.F.: How should I know? After we beat the shit out of him," 'br)
"he won't cross Bloody Fox again.")
               (wait)
               (text-reset 1)
               (text "B.F.: Ha ha ha...ain't that the truth!")
               (wait)
               (text-reset 1)
               (text
                "B.F.: But that fucker just wasted our last dose of Purple" 'br)
"Sky. We gotta go restock.")
               (wait)
               (text-reset 1)
               ...
               (wait)
               (text-reset 1)
               (text "Cole: Damn, I didn't expect to get clubbed like that... I" 'br)
(text "guess that's what happens when you get cocky.")
               (wait)
               (text-reset 1)
               (text
                "Cole: But they were talking about drugs, right? So the stuff" 'br)
"they had wasn't the reanimation serum, it was just typical" 'br)
"street junk, right...?")
               (wait)
               (text-reset 1)
               ...
               (text-reset 1)
               (text
                "Cole: Anyway, no point sitting around. I should head home..." 'br)
"Sheila's probably worried sick.")
               (wait)
               (text-reset 1)
               (text "Cole: Huh?")
               (wait)
               ...
               (text
                "Cole: A matchbook... one of them must have dropped it. Might" 'br)
"come in handy later.")
               (wait)
               (text-reset 1)
 ...
 (seg (? (= P 6))
   (branch-random
    (</>
     (/ (text "Cole: They're attacking that woman, but it's too dark to" 'br)
(text "make out who she is...") (wait) (text-reset 1))
     (/ (text "Cole: They must have kidnapped her from another town...") (wait) (text-reset 1))
     (/
      (text "Woman: Please, help me!")
      (wait)
      (text-reset 1)
      (text "Cole: Sister's pleading for help. I better intervene...")
      (wait)
      (text-reset 1)))))
 (seg (? (= P 2))
   (exec-mem 6064 2 2)
