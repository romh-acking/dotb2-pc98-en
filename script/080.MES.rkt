(mes
 ...
 (seg (? (= P 5) (= 137 #f) (= 140 #f))
   ...
   (text-reset 1))
 (seg (? (= P 5) (= 140 #t) (= 141 #f))
   (text "Lisle: I see... So you're not going to perform the" 'br)
(text "reanimation. In that case, I'm increasingly worried about" 'br)
(text "Sheila's condition...")
   (wait)
   (text-reset 1)
   (text "Cole: Yes... It's not like I'm sacrificing Sheila... But I" 'br)
(text "just think this is something a human should never do.")
   (wait)
   (text-reset 1)
   ...
   (text-reset 1))
 (seg (? (= P 5) (= 140 #t))
   (text "Lisle: Either way, this must have been a difficult decision," 'br)
(text "Cole. You made a good choice.")
   (wait)
   (text-reset 1)
   (text "Cole: ...")
   (wait)
   (text-reset 1))
 (seg (? (= P 5) (= 138 #t))
   (text "Lisle: I'll give my all in resuscitating his brain to ensure" 'br)
(text "that you made the right decision.")
   (wait)
   (text-reset 1)
 ...
 (seg (? (= P 5))
   ...
   (if (</>
        (//
         ...
         (wait)
         (text-reset 1)
         (text "Lisle: I see... You've made a good decision. But I won't be" 'br)
(text "able to relax until Doctor Cooger's brain has been" 'br)
(text "successfully revived.")
         (wait)
         (text-reset 1)
 ...
 (seg (? (= P 6) (= 143 #t))
   ...
   (wait)
   (text-reset 1)
   (text "Cole: He's been hiding out at Razovan Pharmaceuticals. He's" 'br)
(text "been giving orders to Bloody Fox from there.")
   (wait)
   (text-reset 1)
...
"her to Razovan.")
   ...
   (wait)
   (text-reset 1)
   (text "Steve: If we're going to infiltrate the building, we'll need" 'br)
(text "to prep.")
   (wait)
   (text-reset 1)
   (if (</>
        (//
         ...
         (wait)
         (text-reset 1)
         (text "Lisle: That's right. We'll need to collect Doctor Cooger's" 'br)
(text "body from Disbell Cemetery.")
         (wait)
         (text-reset 1)
        ...
        (//
         ...
         (wait)
         (text-reset 1)
         (text "Lisle: Cole... Sheila probably knows what's about to happen." 'br)
(text "Please stay by her side as much as you can.")
         (wait)
         (text-reset 1)
 ...
 (seg (? (= P 6) (= 139 #f) (= 138 #f))
   (text "Steve: Are we going to revive Dr. Cooger, or are we holding" 'br)
(text "off?")
   (set-reg 139 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 6) (= 138 #t) (= 142 #f))
   ...
   (text-reset 1))
 (seg (? (= P 6) (= 138 #t))
   (text "Steve: Either way, it must have been a tough decision... But" 'br)
(text "it's one you made with your heart.")
   (wait)
   (text-reset 1)
   (text "Cole: ...")
   (wait)
   (text-reset 1))
 (seg (? (= P 6) (= 140 #t))
   (text "Steve: Whatever your decision, Cole, I hope it turns out for" 'br)
(text "the best.")
   (wait)
   (text-reset 1))
 (seg (? (= P 6))
   (text "Steve: Hurry up and tell me your decision, Cole. We'll need" 'br)
(text "to prepare regardless.")
   (wait)
   (text-reset 1)
   (if (</>
        (//
         (? (= 134 #t) (= 140 #f))
         (text "Cole: As for reviving Doc... After having a long and hard" 'br)
(text "think, I can't bring myself to do something so inhuman.")
         (wait)
         (text-reset 1)
         (text "Steve: I see. Well, if that's what you want, we'll leave Dr." 'br)
(text "Cooger as he is.")
         (wait)
         (text-reset 1)
         (text "Cole: I'm sorry. Even though you managed to detect brain" 'br)
(text "waves from his body...")
         (wait)
         (text-reset 1)
