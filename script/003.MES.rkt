...
(text "would be losing their shit. But...nothing.")
   ...
   (text-reset 1)
   (text
    "Cole: Even if the culprit is human, no normal person would" 'br)
"chew through Terry and Lynn's skulls and devour their" 'br)
"brains. No... It's gotta be zombies.")
   (set-reg 137 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 5))
...
(text "that reanimation serum came from.")
   ...
   (text-reset 1))
 (seg (? (= P 6))
   (text "Cole: Nobody looking like a culprit in these shops. They may" 'br)
(text "have already made themselves scarce.")
   (wait)
   (text-reset 1))
 (seg (? (= P 7) (= 140 #f))
   (text
    "Cole: Taxis waiting for fares. It's early so there aren't" 'br)
"many yet. By 4AM, it's a veritable cab convoy.")
   (set-reg 140 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 7) (= 141 #f))
...
(text "cabs...")
 ...
 (seg (? (= P 7))
   (text
    "Cole: I could grab a taxi too, but where would I go? Better" 'br)
"to stay here and hunt for clues.")
   (wait)
   (text-reset 1))
 (seg (? (= P 8) (= 142 #f))
...
(text "high-traffic area, so they must have good sales.")
 ...
 (seg (? (= P 8) (= 143 #f))
   (text
    "Cole: That newsstand's run by the regular bearded old fella," 'br)
"hocking papers and magazines. Says he has two grandkids in" 'br)
"the boonies.")
   (set-reg 143 #t)
   (wait)
   (text-reset 1))
...
(text "here.")
 ...
 (seg (? (= P 9) (= 144 #f))
   (text
    "Cole: This bar has outdoor seating. Typically there'd be" 'br)
"three or four people outside drinking, but today it's" 'br)
"unusually empty.")
   (set-reg 144 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 9) (= 145 #f))
   (text
    "Cole: Once I went drinking here with Terry. We got totally" 'br)
"destroyed and drank until the morning.")
   (wait)
   (text-reset 1)
