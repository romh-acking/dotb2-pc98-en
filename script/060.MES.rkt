(mes
 ...
 (seg (? (= P 5) (= 136 #f))
   (text "Cole: There's a large apparatus in the corner of the room. Iwonder what it's used for.")
   (set-reg 136 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 5) (= 137 #f))
   (text "Cole: There's lots of cables coming out of it, so it must bean elaborate piece of equipment.")
   (set-reg 137 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 5) (= 138 #f))
 ...
 (seg (? (= P 6) (= 141 #f))
   ...
   (wait)
   (text-reset 1)
   (text "Cole: Hahaha, not even worth trying. Just seeing that keypadmeans we aren't getting in.")
   (wait)
   (text-reset 1)
   (text "Cole: ...")
   (wait)
 ...
 (seg (? (= P 7) (= 145 #f))
   (text
    "Cole: Back in the day, people used to do stuff by hand, but nowadays, computers handle so much work. Everything's gottenso much more convenient.")
   (set-reg 145 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 7))
 ...
 (seg (? (= P 1))
   (exec-mem 6064 2 1)
   (text "Cole: Nobody's around, so I might as well head back into thehallway.")
   (wait)
   (text-reset 1)
   (exec-mem 6064 3)
   (sound '|| 2)
