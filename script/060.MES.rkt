(mes
 ...
 (seg (? (= P 5) (= 136 #f))
   (str "Cole: There's a large apparatus in the corner of the room. I" 'br)
(str "wonder what it's used for.")
   (set-reg 136 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 5) (= 137 #f))
   (str "Cole: There's lots of cables coming out of it, so it must be" 'br)
(str "an elaborate piece of equipment.")
   (set-reg 137 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 5) (= 138 #f))
...
(str "to open it.")
   ...
   (wait)
   (text-reset 1)
   (str "Cole: Hahaha, not even worth trying. Just seeing that keypad" 'br)
(str "means we aren't getting in.")
   (wait)
   (text-reset 1)
   (str "Cole: ...")
   (wait)
...
(str "tank.")
 ...
 (seg (? (= P 1))
   (exec-mem 6064 2 1)
   (str "Cole: Nobody's around, so I might as well head back into the" 'br)
(str "hallway.")
   (wait)
   (text-reset 1)
   (exec-mem 6064 3)
