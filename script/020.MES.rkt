(mes
 ...
 (seg (? (= P 5) (= 125 #f))
   ...
   (text-reset 1)
   (text
    "Cole: When you think of a police station, you'd imagine crooks coming and going, but in a small town like ours, this is basically all it amounts to.")
   (set-reg 125 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 5) (= 126 #f))
 ...
 (seg (? (= P 8) (= 133 #f))
   ...
   (wait)
   (text-reset 1)
   (text "Cole: Aw man, if something like that went down, my apartment would be flattened.")
   (set-reg 133 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 8))
