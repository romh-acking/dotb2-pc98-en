 ...
 (seg*
  ...
  (if (</>
       (//
        ...
        (str "Cole: Ah, it's already dark out... I finally must have" 'br)
        (str "slept for a while.")
        (wait)
        (text-reset 1)
        ...
        (text-reset 1))
       (//
        (str "Cole: This is my room at the Institute. It's still too" 'br)	; Lead-in from second visit onwards
        (str "early to sleep.")
        (wait)
        (text-reset 1))))
  (loop (exec-mem 3744 3 "P" 32768) (text-color 15) (seg-call)))
 (seg (? (= P 5) (= 101 #f))							; Lockers #1
   (str "Cole: These lockers are locked. I can't get into any of" 'br)
   (str "them.")
   (set-reg 101 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 5))								; Lockers #2 (repeat line)
   (str "Cole: I don't have anything of value, so I have no need for" 'br)
   (str "a locker.")
   (wait)
   (text-reset 1))
 (seg (? (= P 6) (= 102 #f))							; Door #1
   (str "Cole: The corridor's behind this door.")
   (set-reg 102 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 6))								; Door #2 (repeat line)
   (str "Cole: Since there's no lock on this door, if any zombies" 'br)
   (str "get in, it's game over.")
   (wait)
   (text-reset 1))
 (seg (? (= P 7) (= 103 #f))							; Desk #1
   (str "Cole: There's a desk on the left. I feel like I've bashed" 'br)
   (str "my head into it more than a few times while sleeping.")
   (set-reg 103 #t)
   (wait)
   (text-reset 1))
