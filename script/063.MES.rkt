...
(str "to sleep.") (wait) (text-reset 1))))
...
(str "them.")
   ...
   (wait)
   (text-reset 1))
 (seg (? (= P 5)) (str "Cole: I don't have anything of value, so I have no need for" 'br)
(str "a locker.") (wait) (text-reset 1))
 (seg (? (= P 6) (= 102 #f))
...
(str "head into it more than a few times while sleeping.")
   ...
   (wait)
   (text-reset 1))
 (seg (? (= P 7)) (str "Cole: This desk doesn't serve much purpose aside from" 'br)
(str "colliding with my head while I'm sleeping.") (wait) (text-reset 1))
 (seg (? (= P 1))
   (exec-mem 6064 2 1)
