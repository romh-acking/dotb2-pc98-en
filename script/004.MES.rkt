(mes
 ...
 (seg (? (= P 5) (= 148 #f))
   ...
   (text-reset 1))
 (seg (? (= P 5))
   (text "Cole: Nobody's helping her because even if you manage to" 'br)
   (text "land a hit on those guys, they'll come back with their" 'br)
   (text "friends later for revenge.")
   (wait)
   (text-reset 1)
 ...
 (seg (? (= P 6) (= 150 #t) (= 154 #t) (= 155 #f))
   (text "Cole: Wait a minute... Right there, is that paint on the" 'br)
   (text "bike tire? It looks like the droplets of the reanimation" 'br)
   (text "serum from the alley. Are these the guys?")
   (wait)
   (text-reset 1)
   (text "Cole: No, no. Those guys just got here, and we couldn't" 'br)
   (text "hear any bikes after the murders.")
   (set-reg 155 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 6) (= 150 #t) (= 154 #t))
   (text "Cole: Maybe it's just paint. I'll ask...and maybe see if I" 'br)
   (text "can help the girl too.")
   (wait)
   (text-reset 1)
 ...
 (seg (? (= P 6) (= 149 #f))
   (text "Cole: That's gotta be one of his buddies on his bike. I" 'br)
   (text "guess that's the motorcycle I heard earlier.")
   (set-reg 149 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 6) (= 150 #f))
   ...
   (text-reset 1))
 (seg (? (= P 6))
   (text "Cole: If you're from around here, this kinda scene isn't" 'br)
   (text "uncommon. It was already Bloody Fox's territory when I" 'br)
   (text "moved in.")
   (wait)
   (text-reset 1))
 (seg (? (= P 7) (= 151 #f))
   ...
   (wait)
   (text-reset 1)
   (text "Cole: But if you throw down with Bloody Fox, you're" 'br)
   (text "cruising for a bruising.")
   (set-reg 151 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 7) (= 152 #f))
 ...
 (seg (? (= P 7) (= 153 #f))
   ...
   (wait)
   (text-reset 1)
   (text "B.F.: Wahahaha! The cops answer to us, dipshit, they aren't" 'br)
   (text "doing squat! Why don't you come have some fun with us?")
   (wait)
   (text-reset 1)
   (text "Cole: ...")
   (set-reg 153 #t)
 ...
 (seg (? (= P 7) (= 154 #f))
   ...
   (wait)
   (text-reset 1)
   (text "Cole: I feel sorry for her, but he's a Bloody Fox member." 'br)
   (text "If I get involved, there's gonna be trouble.")
   (set-reg 154 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 7))
