(mes
 ...
 (seg*
  ...
  (if (</>
       (//
        (? (= 100 #f))
        (str "Steve: Oh, there's Cole. You're just in time. How are things" 'br)
(str "looking? Could you clean out the first floor?")
        (wait)
        (text-reset 1)
        ...
        (wait)
        (text-reset 1))
       (// (str "Steve: Cole...") (wait) (text-reset 1))))
...
(str "him questions, and...")
   ...
   (wait)
   (text-reset 1)
   (str "Steve: What a man... So, does that mean the person who broke" 'br)
(str "open the entrance...")
   (wait)
   (text-reset 1)
...
"brain.")
   (wait)
   (text-reset 1)
   (str "Steve: Ah, at that range, even a toddler could have made the" 'br)
(str "shot.")
   (set-reg 105 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 6)) (str "Steve: So what happened on the first floor?") (wait) (text-reset 1))
 (seg (? (= P 7)) (str "Lisle: I'll be in the lab all day.") (wait) (text-reset 1))
 (seg (? (= P 1) (= 107 #t))
   (exec-mem 6064 2 1)
