 ...
 (seg*
  ...
  (if (</>
       ...
       (//
        ...
        (text "Cole: I definitely saw someone. I gotta check out the" 'br)
(text "restroom.")
        (wait)
        (text-reset 1))
       (//
 ...
 (seg (? (= P 5))
   ...
   (mes-jump "A:¥MES¥069.MES"))
 (seg (? (= P 6) (= 130 #f))
   (text "Cole: Steve said he blasted a zombie in the front office." 'br)
(text "Should I make sure it's really dead?")
   (set-reg 130 #t)
   (wait)
   (text-reset 1)
   (exec-mem 9920 2 6 6 113 64 6 32 113 32 0 50)
 ...
 (seg (? (= P 6))
   (text "Cole: This is the front office. Those are definitely some" 'br)
(text "dead zombies courtesy of Steve... Nice shootin', Tex.")
   (wait)
   (text-reset 1)
   (exec-mem 9920 2 6 6 113 64 6 32 113 32 0 50)
   (nop@)
   ...
   (mes-jump "A:¥MES¥070.MES"))
 (seg (? (= P 7) (= 131 #f))
   (text "Cole: No need to check the elevator, since the power's been" 'br)
(text "cut.")
   (set-reg 131 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 7))
   (text "Cole: The elevator hasn't got power, so there's no need to" 'br)
(text "check it.")
   (wait)
   (text-reset 1))
 (seg (?
       ...
       (= 156 #t)
       (= 164 #f))
   (text "Cole: My eyes weren't deceiving me... Someone definitely" 'br)
(text "went into that restroom. Better keep my eyes peeled.")
   (wait)
   (text-reset 1)
   (exec-mem 9920 2 6 6 113 64 6 32 113 32 0 50)
   (nop@)
