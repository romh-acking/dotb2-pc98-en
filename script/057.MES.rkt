  ...
  (if (</>
       (//
        (?
         ...
         (= 142 #t)
         (= 146 #t))
        (text "Cole: I'm starting to tire out... It's about time I got some" 'br)
(text "rest.")
        (wait)
        (text-reset 1)
        ...
        (wait)
        (text-reset 1)
        (text "Cole: Day and night... have totally reversed on my body" 'br)
(text "clock.")
        (wait)
        (text-reset 1)
        (text "Cole: Sleepy...")
        ...
        (wait)
        (text-reset 1)
        (text "Catherine: You were tossing and turning so much, and" 'br)
(text "sweating like crazy. ")
        (wait)
        (text-reset 1)
        ...
        (wait)
        (text-reset 1)
        (text "Catherine: I couldn't get to sleep. When I stepped into the" 'br)
(text "hallway to take some tranquilizers, I heard moaning from" 'br)
(text "your room.")
        (wait)
        (text-reset 1)
        ...
        (wait)
        (text-reset 1)
        (text "Cole: Catherine, there was something you wanted to talk" 'br)
(text "about, right?")
        (wait)
        (text-reset 1)
        (text "Catherine: ...")
        (wait)
        (text-reset 1)
        (text "Cole: How about we head up to the roof? I'd been wanting to" 'br)
(text "check it out.")
        (wait)
        (text-reset 1)
 ...
 (seg (? (= P 5) (= 114 #f))
   (text "Cole: There are rows of lockers with locks on them. Probably" 'br)
(text "used by the people working here.")
   (set-reg 114 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 5) (= 115 #f))
   ...
   (wait)
   (text-reset 1))
 (seg (? (= P 5)) (text "Cole: These are the institute's lockers. I can leave them" 'br)
(text "be.") (wait) (text-reset 1))
 (seg (? (= P 6) (= 116 #f))
 ...
 (seg (? (= P 6))
   ...
   (text-reset 1))
 (seg (? (= P 7) (= 118 #f))
   (text "Cole: There's a desk in here. I used to study a lot, but" 'br)
(text "nowadays I hardly ever use a desk.")
   (set-reg 118 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 7) (= 119 #f))
   (text "Cole: The drawer's empty. Nothing in here to grab my" 'br)
(text "attention.")
   (set-reg 119 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 7))
   (text "Cole: I need to make sure I don't bash my head on the edge" 'br)
(text "of the desk when I roll over in bed.")
   (wait)
   (text-reset 1))
 (seg (? (= P 1))
   (exec-mem 6064 2 1)
