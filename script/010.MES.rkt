(mes
 ...
 (seg (? (= P 5) (= 196 #f))
   (text
    "Cole: This is the same Bar Rease that was mentioned in the newspaper article, where the gang member killed seven of his friends.")
   (wait)
   (text-reset 1)
 ...
 (seg (? (= P 5) (= 197 #f))
   (text
    "Cole: I'm sure the Purple Sky drug has been doing the rounds here. Got a hunch Ben might have something to do with it, too.")
   (set-reg 197 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 5))
   ...
   (text-reset 1))
 (seg (? (= P 6) (= 198 #f))
   (text "Cole: The entrance to Bar Rease. The door's at the bottom of the stairs.")
   (set-reg 198 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 6) (= 199 #f))
 ...
 (seg (? (= P 7) (= 200 #f))
   ...
   (text-reset 1))
 (seg (? (= P 7) (= 201 #f))
   (text "B.F.: Watch out you don't slip on any leftover brains on the ground, you're standin' where Ben got his head blasted open! Heh heh heh...")
   (wait)
   (text-reset 1)
 ...
 (seg (? (= P 7))
   ...
   (text-reset 1))
 (seg (? (= P 8) (= 203 #f))
   (text "Cole: That's a lot of graffiti... Looks like a New York subway. I guess the bar owner figures the F-bombs add to the ambience.")
   (set-reg 203 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 8)) (text "Cole: A bare concrete wall. Urban brutalism at its finest.") (wait) (text-reset 1))
 (seg (? (= P 9) (= 204 #f))
   (text "Cole: A bunch of Bloody Fox bikes parked outside. The owners are probably inside, sticking to Shirley Temples.")
   (set-reg 204 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 9))
   (text "Cole: They aren't taking good care of these bikes at all, they're filthy! The resale value on these things is gonna be nil.")
   (wait)
   (text-reset 1))
 (seg (? (= P 1) (= 202 #t) (= 204 #t))
   (exec-mem 6064 2 1)
