(mes
 ...
 (seg (? (= P 5) (= 140 #f))
   (text
    "Cole: Doc had tripped over something and dropped the bag" 'br)
"here, and the impact broke the reanimation serum's flask.")
   (set-reg 140 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 5) (= 141 #f))
   (text "Cole: If nobody cleaned up the shards, then no one's been" 'br)
(text "here for the last two years...")
   (set-reg 141 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 5))
   (text "Cole: This is a glass shard from the vial that contained the" 'br)
(text "reanimation serum.")
   (wait)
   (text-reset 1))
 (seg (? (= P 6) (= 142 #f))
   (text "Cole: No doubt about it, that's Doc's bag. Amazing it's" 'br)
(text "still here.")
   (set-reg 142 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 6) (= 143 #f))
   (text "Cole: The cure's supposed to be in that bag. But I hope the" 'br)
(text "container wasn't broken...")
   (set-reg 143 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 6))
   ...
   (wait)
   (text-reset 1)
   (text "Cole: Huh, there's a notebook in here too... I'll take it" 'br)
(text "with me, just in case.")
   (wait)
   (text-reset 1)
   (exec-mem 912 "PUT 1 54 38,W 0,O 1")
   (exec-mem 912 "EXIT")
   (text "Cole: Now I can save Sheila! Okay, I gotta get back to the" 'br)
(text "lab!")
   (wait)
   (text-reset 1)
   (sound '|| 2)
 ...
 (seg (? (= P 7) (= 144 #f))
   (text "Cole: The upkeep here's dismal. The graves are in a filthy" 'br)
(text "state.")
   (set-reg 144 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 7) (= 145 #f))
   (text "Cole: The rear of a tombstone. The name must be carved on" 'br)
(text "the other side.")
   (set-reg 145 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 7))
   (text "Cole: If I remember correctly, when Doc and I were running" 'br)
(text "away, the flask with the serum hit this tombstone and" 'br)
(text "shattered...")
   (wait)
   (text-reset 1))
 (seg (? (= P 8) (= 146 #f))
