(mes
 ...
 (seg*
  ...
  (if (</>
       (//
        (? (= 108 #f))
        (text "Cole: The stench of death is thick in here. I need to do what I came to do and get out.")
        (wait)
        (text-reset 1)
       ...
       (//
        (text
         "Cole: This is the ground floor of the 32nd Precinct. There's bodies of cops and civilians everywhere. I gotta find Steve and bounce.")
        (wait)
        (text-reset 1))))
  (loop (exec-mem 3744 3 "P" 32768) (text-color 15) (seg-call)))
 (seg (? (= P 5) (= 109 #f))
   (text "Cole: It looks like some of the bodies were cops. Not one of 'em is holding a gun.")
   (wait)
   (text-reset 1)
   (text "Cole: They probably didn't even have a chance to draw their weapons once the zombies attacked.")
   (set-reg 109 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 5) (= 110 #f))
   (text
    "Cole: The horrific funk from these bodies is steaming up the room. If it stays like this, we're risking an epidemic.")
   (wait)
   (text-reset 1)
   (text
    "Cole: Steve said it'd be three days until the military arrived, but by then, that bastard Daniel will be long gone.")
   (wait)
   (text-reset 1)
   ...
   (text-reset 1))
 (seg (? (= P 5) (= 111 #f))
   (text "Cole: Just like two years ago, no matter how many bodies I see, I can't get used to it.")
   (set-reg 111 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 5))
   (text
    "Cole: Among the bodies, there are some women... Those zombies want brains, and they'll attack anyone regardless of sex.")
   (set-reg 112 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 6) (= 113 #f))
   (text "Cole: There's a drink vending machine on the left. It won't be vending again.")
   (set-reg 113 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 6) (= 114 #f))
   (text
    "Cole: There's a big gash on the vending machine, like it was hit by something. Makes you wonder what went down when the zombies attacked.")
   (set-reg 114 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 6))
   ...
   (text-reset 1))
 (seg (? (= P 7) (= 115 #f))
   (text "Cole: The entrance I came through. The glass door's been shattered.")
   (set-reg 115 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 7) (= 116 #f))
   (text "Cole: That door's the only way in and out. If the zombies attack, there's no other exit.")
   (set-reg 116 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 7))
   ...
   (text-reset 1))
 (seg (? (= P 8) (= 117 #f))
   (text "Cole: These are the stairs to the second floor. I went up these when I visited Steve's Investigation Division.")
   (set-reg 117 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 8) (= 118 #f))
   (text
    "Cole: Steve must have come here to grab the relevant files. He's probably upstairs in the Investigation Division, searching for them now.")
   (set-reg 118 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 8))
   (text "Cole: The Investigation Division is up the stairs. Steve's desk is there too.")
   (wait)
   (text-reset 1))
 (seg (? (= P 1) (= 112 #t) (= 116 #t) (= 118 #t))
   (exec-mem 6064 2 1)
   (text "Cole: Okay, let's head up to the second floor. Steve should be in the Investigation Division.")
   (wait)
   (text-reset 1)
   (text "Cole: Wha?")
   (wait)
   (text-reset 1)
   (text "Cole: Just now, it looked like that corpse shifted slightly... Or was that just my imagination?")
   (wait)
   (text-reset 1)
   (text "Cole: Oh shit--")
   (wait)
   ...
   (if (</>
        (//
         (? (= P 1))
         (text "Cole: Shit! I managed to take some down, but I'm out of bullets!")
         (wait)
         (text-reset 1)
