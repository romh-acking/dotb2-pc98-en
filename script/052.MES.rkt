...
(str "what I came to do and get out.")
        ...
        (text-reset 1))
       (//
        (str "Cole: This is the ground floor of the 32nd Precinct. There's" 'br)
(str "bodies of cops and civilians everywhere. I gotta find Steve" 'br)
(str "and bounce.")
        (wait)
        (text-reset 1))))
...
(str "weapons once the zombies attacked.")
   ...
   (text-reset 1))
 (seg (? (= P 5) (= 110 #f))
   (str "Cole: The horrific funk from these bodies is steaming up the" 'br)
(str "the room. If it stays like this, we're risking an epidemic.")
   (wait)
   (text-reset 1)
   (str "Cole: Steve said it'd be three days until the military" 'br)
(str "arrived, but by then, that bastard Daniel will be long gone.")
   (wait)
   (text-reset 1)
...
(str "was hit by something. Makes you wonder what went down" 'br)
(str "when the zombies attacked.")
   (set-reg 114 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 6))
