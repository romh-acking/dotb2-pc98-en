(mes
 ...
 (seg (? (= P 5) (= 125 #f))
   (text "Cole: There's just one bed in here. The sheets aren't" 'br)
(text "wrinkled, so they must have been changed recently.")
   (set-reg 125 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 5) (= 126 #f))
 ...
 (seg (? (= P 5))
   ...
   (text-reset 1))
 (seg (? (= P 6) (= 127 #f))
   (text "Cole: It's pitch black outside. The streets are probably" 'br)
(text "crawling with those brain-sucking zombie freaks by now.")
   (set-reg 127 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 6) (= 128 #f))
   (text "Cole: We're on the second floor, and zombies can't climb up" 'br)
(text "here. Probably.")
   (set-reg 128 #t)
   (wait)
   (text-reset 1))
 ...
 (seg (? (= P 7) (= 129 #f))
   ...
   (text-reset 1))
 (seg (? (= P 7) (= 130 #f))
   (text "Cole: If the calendar's still on July, that means someone" 'br)
(text "was using this room four months ago.")
   (set-reg 130 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 7) (= 131 #f))
 ...
 (seg (? (= P 1))
   (exec-mem 6064 2 1)
   (text "Cole: Nothing of interest in this room, let's go back to the" 'br)
(text "hallway.")
   (wait)
   (text-reset 1)
   (exec-mem 6064 3)
