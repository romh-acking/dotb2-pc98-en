(mes
 ...
 (seg (? (= P 8) (= 113 #f))
   ...
   (wait)
   (text-reset 1)
   (text "［スティーブ］悪いが、もう休ませてくれないか。明日も色々と忙　　　　　　　しいのでな・・・。")
   (wait)
   (text-reset 1)
   (text "［コール］あ、はい。おやすみなさい。")
   (wait)
   (text-reset 1)
   (text "［コール］スティーブさん、かなり疲れているようだな・・・。")
   (set-reg 113 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 8))
   (text "［コール］ここはスティーブさんの部屋だが、彼はかなり疲れてい　　　　　るようだから今日はもう寝かせてあげた方がいいだろう　　　　　な。")
   (wait)
   (text-reset 1))
 (seg (? (= P 9))
   (text "［コール］研究室へ行ってみよう。")
