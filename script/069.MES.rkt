(mes
 ...
 (seg (? (= P 5) (= 134 #f))
   (text "Cole: The stairs are unstable, so I can't let myself get cornered here. If I lost my footing, I'd be screwed.")
   (set-reg 134 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 5) (= 135 #f))
   ...
   (text-reset 1))
 (seg (? (= P 5))
   (text "Cole: I can only hear the sound of my footsteps echoing on the stairs. There are no zombies around.")
   (wait)
   (text-reset 1))
 (seg (? (= P 6) (= 136 #f))
   (text "Cole: The shutter's sealed. Steve said he had closed them all yesterday.")
   (wait)
   (text-reset 1)
   ...
   (text-reset 1))
 (seg (? (= P 6) (= 137 #f))
   (text "Cole: Since the shutter's locked in place, I guess we can assume no zombies made it past here.")
   (set-reg 137 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 6))
   (text "Cole: There's a red emergency light over the shutter. I can't go any further.")
   (wait)
   (text-reset 1))
 (seg (? (= P 1) (= 138 #f))
   (exec-mem 6064 2 1)
   ...
   (if (</>
        (//
         ...
         (exec-mem 9920 2 6 6 113 64 6 32 113 32 0 50)
         (text
          "Cole: That was close... I don't want to take wild shots from somewhere with unstable footing. If I blew it, I might have taken a fall down the stairs, and...I'd be dinner.")
         (wait)
         (text-reset 1)
         (sound '|| 2)
         (nop@)
         ...
         (branch-var
          ...
          (</>
           ...
           (/
            ...
            (loop (wait)))))))))
 (seg (? (= P 1))
   (text "Cole: I can't go back to the first floor without having to look at the zombie I blasted earlier... I hate this part.")
   (wait)
   (text-reset 1)
   (nop@)
   (set-reg 127 #t)
