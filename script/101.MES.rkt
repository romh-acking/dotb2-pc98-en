(mes
 ...
 (seg*
  ...
  (if (</>
       (//
        (? (= 128 #f))
        (text "Cole: I doubled back onto the same path as before, but wheredid I take a wrong turn...?")
        (set-reg 128 #t))
 ...
 (seg (? (= P 6) (= 134 #f))
   ...
   (branch-var
    ...
    (</>
     (/
      (text "Cole: Wait a second. If Steve came to help me, why didn't hemake himself known?")
      (wait)
      (text-reset 1)
      (text "Cole: It's not like he's got a reason to hide from me. Thoseprints have to belong to someone else.")
      (wait)
      (text-reset 1))
     (/
      (text
     ...
     (/
      ...
      (wait)
      (text-reset 1)
      (text "Cole: Could it be that there's other androids like Ghoul outthere?")
      (set-reg 134 #t)
      (wait)
      (text-reset 1)))))
 (seg (? (= P 6) (= 135 #f))
 ...
 (seg (? (= P 7) (= 136 #f))
   ...
   (text-reset 1))
 (seg (? (= P 7) (= 137 #f))
   (text "Cole: I know it's not this way, but there doesn't seem to beanother path around here.")
   (set-reg 137 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 7))
 ...
 (seg (? (= P 1) (= 137 #t) (= 135 #t))
   ...
   (wait)
   (text-reset 1)
   (text "Cole: I've got to double back and try again. I can't go backto the lab empty-handed.")
   (wait)
   (text-reset 1)
   (text "Cole: Oh!")
   (wait)
