...
(str "reception area.")
        (set-reg 159 #t))
       (// (str "Cole: This is the first-floor area of Razovan" 'br)
(str "Pharmaceuticals."))))
  (wait)
  (text-reset 1)
...
(str "Nobody's been keeping an eye on the furniture.")
   ...
   (text-reset 1))
 (seg (? (= P 6))
   (str "Cole: This sofa's filthy... If you sat down on it, your butt" 'br)
(str "would turn white from the dust.")
   (wait)
   (text-reset 1))
 (seg (? (= P 7) (= 164 #f))
...
"elevator, so it's the stairs or nothing.")
 ...
 (seg (? (= P 8) (= 167 #f))
   ...
   (wait)
   (text-reset 1)
   (str "Cole: There are two sets of footprints, a larger and smaller" 'br)
(str "pair. The larger ones are probably Steve's.")
   (set-reg 167 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 8))
