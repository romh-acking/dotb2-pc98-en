 ...
 (seg*
  ...
  (if (</>
       (//
        ...
        (wait)
        (text-reset 1)
        (text "Steve: I've heard the stories, but this is my first time" 'br)
(text "here.")
        (set-reg 100 #t)
        (wait)
        (text-reset 1))
  ...
  (loop
   ...
   (seg-call)))
 (seg (? (= P 5) (= 101 #f))
   (text "Cole: This building's in rough shape. I can't believe this" 'br)
(text "is Bloody Fox's HQ.")
   (set-reg 101 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 5) (= 102 #f))
   (text
    "Steve: I heard the building's owner is Japanese. He was" 'br)
"taking his sweet time trying to sell it, then Bloody Fox" 'br)
"just swept in and claimed it for themselves.")
   (set-reg 102 #t)
   (wait)
   (text-reset 1)
   (text "Cole: Poor guy.")
 ...
 (seg (? (= P 6) (= 104 #f))
   (text "Cole: That's the entrance to the building, but it sure looks" 'br)
(text "like it's falling apart.")
   (set-reg 104 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 6) (= 105 #f))
 ...
 (seg (? (= P 6))
   ...
   (wait)
   (text-reset 1)
   (text "Steve: Probably.. If there are Bloody Fox members inside, we" 'br)
(text "won't be able to get out unless we take them all down.")
   (wait)
   (text-reset 1)
   (text "Cole: Those guys have a strong sense of camaraderie, don't" 'br)
(text "they.")
   (wait)
   (text-reset 1))
 (seg (? (= P 7) (= 106 #f))
 ...
 (seg (? (= P 7) (= 107 #f))
   ...
   (text-reset 1))
 (seg (? (= P 7))
   (text "Cole: I see lightning... We shouldn't stay outside much" 'br)
(text "longer.")
   (wait)
   (text-reset 1)
