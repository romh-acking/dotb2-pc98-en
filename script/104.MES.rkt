(mes
 ...
 (seg (? (= P 6) (= 104 #f))
   (text "Cole: That's the entrance to the building, but it sure lookslike it's falling apart.")
   (set-reg 104 #t)
   (wait)
   (text-reset 1))
 (seg (? (= P 6) (= 105 #f))
 ...
 (seg (? (= P 6))
   ...
   (wait)
   (text-reset 1)
   (text "Steve: Probably.. If there are Bloody Fox members inside, wewon't be able to get out unless we take them all down.")
   (wait)
   (text-reset 1)
